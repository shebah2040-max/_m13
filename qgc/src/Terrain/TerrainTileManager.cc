...
static QString _terrainDiskCachePath(const QString &hash)
{
    const QString cacheDir = QStandardPaths::writableLocation(QStandardPaths::CacheLocation) + QStringLiteral("/terrain-tiles");
    (void) QDir().mkpath(cacheDir);
    return cacheDir + QLatin1Char('/') + hash + QStringLiteral(".bin");
}
...
void TerrainTileManager::_cacheTile(const QByteArray &data, const QString &hash)
{
    ...
    // Persist to disk so the tile survives across sessions
    const QString diskPath = _terrainDiskCachePath(hash);
    if (!QFile::exists(diskPath)) {
        QFile f(diskPath);
        if (f.open(QIODevice::WriteOnly)) {
            (void) f.write(data);
        } else {
            qCWarning(TerrainTileManagerLog) << "Could not write terrain tile to disk cache:" << diskPath;
        }
    }
...
{
    {
        ...
        if (_tiles.contains(hash)) {
            ...
            return tile->isValid() ? tile : nullptr;
        }
    }

    // Try loading from persistent disk cache before giving up
    const QString diskPath = _terrainDiskCachePath(hash);
    QFile f(diskPath);
    if (f.exists() && f.open(QIODevice::ReadOnly)) {
        const QByteArray data = f.readAll();
        TerrainTile* const tile = new TerrainTile(data);
        if (tile->isValid()) {
            qCDebug(TerrainTileManagerLog) << "Loaded terrain tile from disk cache:" << hash;
            QMutexLocker locker(&_tilesMutex);
            if (!_tiles.contains(hash)) {
                (void) _tiles.insert(hash, tile);
            } else {
                delete tile;
            }
            return _tiles[hash];
        ...
        delete tile;
        // Corrupt disk cache entry — remove it
        f.remove();
    }
