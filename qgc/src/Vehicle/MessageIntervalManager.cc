...
void MessageIntervalManager::configureHighRateTelemetry()
{
    // Request flight-critical messages at 10 Hz immediately after vehicle connection
    // for higher-precision attitude, position, and IMU monitoring.
    static constexpr uint16_t kHighRateMsgIds[] = {
        30,  // MAVLINK_MSG_ID_ATTITUDE
        33,  // MAVLINK_MSG_ID_GLOBAL_POSITION_INT
        26,  // MAVLINK_MSG_ID_SCALED_IMU
        74,  // MAVLINK_MSG_ID_VFR_HUD
    };
    static constexpr int32_t kHighRateHz = 10;

    const uint8_t apCompId = MAV_COMP_ID_AUTOPILOT1;
    for (const uint16_t msgId : kHighRateMsgIds) {
        setMessageRate(apCompId, msgId, kHighRateHz);
    }
}
